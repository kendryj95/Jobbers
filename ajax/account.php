<?php
	...
	switch($op) {
		case ADD_TO_CURRICULUM:
			...
			switch($_REQUEST["opt"]) {
				case 1:
					...
					$db->query("UPDATE trabajadores SET id_sexo='$_REQUEST[sex]', id_estado_civil='$_REQUEST[estadoCivil]', id_tipo_documento_identificacion='$_REQUEST[dni]', id_pais='$_REQUEST[country]', provincia='$_REQUEST[province]', localidad='$_REQUEST[city]', calle='$_REQUEST[street]', nombres='$_REQUEST[name]',  apellidos='$_REQUEST[lastName]', numero_documento_identificacion='$_REQUEST[numberdni]', fecha_nacimiento='".date('Y-m-d', strtotime($_REQUEST['birthday']))."', telefono='$_REQUEST[phone]', telefono_alternativo='$_REQUEST[phoneAlt]', sitio_web ='$_REQUEST[sitio_web]', facebook='$_REQUEST[fb]', twitter='$_REQUEST[tw]', instagram='$_REQUEST[ig]', snapchat ='$_REQUEST[snap]', linkedin='$_REQUEST[lkd]', fecha_actualizacion='".date('Y-m-d h:i:s')."' WHERE id=".$_SESSION["ctc"]["id"]);
					$data = array();
				...
				case 2:
					if($id != "" && $id != "undefined") {
						$db->query("UPDATE trabajadores_experiencia_laboral SET nombre_empresa='$_REQUEST[company]', id_pais='$_REQUEST[rCompany]', id_actividad_empresa='$_REQUEST[tCompany]', tipo_puesto='$_REQUEST[tEmployeer]', mes_ingreso='$_REQUEST[monthI]', ano_ingreso='$_REQUEST[yearI]', mes_egreso='$_REQUEST[monthE]', ano_egreso='$_REQUEST[yearE]', nombre_encargado='$_REQUEST[nom_enc]', tlf_encargado='$_REQUEST[tlf_enc]', descripcion_tareas='$_REQUEST[descriptionArea]' WHERE id=$_REQUEST[i]");
					}
					else {
						...
						$db->query("INSERT INTO trabajadores_experiencia_laboral (id_trabajador, nombre_empresa, id_pais, id_actividad_empresa, tipo_puesto, mes_ingreso, ano_ingreso, mes_egreso, ano_egreso, nombre_encargado, tlf_encargado, descripcion_tareas) VALUES ('".$_SESSION["ctc"]["id"]."', '$_REQUEST[company]', '$_REQUEST[rCompany]', '$_REQUEST[tCompany]', '$_REQUEST[tEmployeer]', '$_REQUEST[monthI]', '$_REQUEST[yearI]', '$_REQUEST[monthE]', '$_REQUEST[yearE]', '$_REQUEST[nom_enc]', '$_REQUEST[tlf_enc]', '$_REQUEST[descriptionArea]')");
					}
		...
		case LOAD_CURRICULUM:
			$data["usuario"] = $db->getRow("SELECT trabajadores.nombres, trabajadores.apellidos, trabajadores.correo_electronico, trabajadores.fecha_nacimiento, trabajadores.telefono, trabajadores.telefono_alternativo, paises.nombre as pais FROM trabajadores INNER JOIN paises ON paises.id=trabajadores.id_pais WHERE trabajadores.id=".$_SESSION["ctc"]["id"]);
