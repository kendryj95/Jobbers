<?php
	...
	switch($op) {
		case ADD_TO_CURRICULUM:
			...
			switch($_REQUEST["opt"]) {
				case 1:
					$db->query("UPDATE trabajadores SET id_sexo='$_REQUEST[sex]', id_estado_civil='$_REQUEST[estadoCivil]', id_tipo_documento_identificacion='$_REQUEST[dni]', id_pais='$_REQUEST[country]', provincia='$_REQUEST[province]', localidad='$_REQUEST[city]', calle='$_REQUEST[street]', nombres='$_REQUEST[name]',  apellidos='$_REQUEST[lastName]', numero_documento_identificacion='$_REQUEST[numberdni]', fecha_nacimiento='".date('Y-m-d', strtotime($_REQUEST['birthday']))."', telefono='$_REQUEST[phone]', telefono_alternativo='$_REQUEST[phoneAlt]', fecha_actualizacion='".date('Y-m-d h:i:s')."' WHERE id=".$_SESSION["ctc"]["id"]);
					$data = array();
				...
				case 2:
					if($id != "" && $id != "undefined") {
						$db->query("UPDATE trabajadores_experiencia_laboral SET nombre_empresa='$_REQUEST[company]', id_pais='$_REQUEST[rCompany]', id_actividad_empresa='$_REQUEST[tCompany]', tipo_puesto='$_REQUEST[tEmployeer]', mes_ingreso='$_REQUEST[monthI]', ano_ingreso='$_REQUEST[yearI]', mes_egreso='$_REQUEST[monthE]', ano_egreso='$_REQUEST[yearE]', descripcion_tareas='$_REQUEST[descriptionArea]' WHERE id=$_REQUEST[i]");
					}
					else {
						...
						$db->query("INSERT INTO trabajadores_experiencia_laboral (id_trabajador, nombre_empresa, id_pais, id_actividad_empresa, tipo_puesto, mes_ingreso, ano_ingreso, mes_egreso, ano_egreso, descripcion_tareas) VALUES ('".$_SESSION["ctc"]["id"]."', '$_REQUEST[company]', '$_REQUEST[rCompany]', '$_REQUEST[tCompany]', '$_REQUEST[tEmployeer]', '$_REQUEST[monthI]', '$_REQUEST[yearI]', '$_REQUEST[monthE]', '$_REQUEST[yearE]', '$_REQUEST[descriptionArea]')");
					}
				...
				case 6:
					if($id != "" && $id != "undefined") {
						$db->query("UPDATE trabajadores_infextra SET remuneracion_pret=$_REQUEST[remuneracion], objetivo_laboral='$_REQUEST[objLab]', carta_present = '$_REQUEST[cartaPres]' WHERE id_trabajador=$_REQUEST[i]");
						...
						$db->query("INSERT INTO trabajadores_infextra (id_trabajador, remuneracion_pret, objetivo_laboral, carta_present) VALUES ('".$_SESSION["ctc"]["id"]."', $_REQUEST[remuneracion], '$_REQUEST[objLab]', '$_REQUEST[cartaPres]')");
						$data = $db->getAll("SELECT * FROM trabajadores_infextra WHERE id=$id");
					}
					break;
			}
		...
		case LOAD_CURRICULUM:
			$data["usuario"] = $db->getRow("SELECT trabajadores.nombres, trabajadores.apellidos, trabajadores.correo_electronico, trabajadores.fecha_nacimiento, paises.nombre as pais FROM trabajadores INNER JOIN paises ON paises.id=trabajadores.id_pais WHERE trabajadores.id=".$_SESSION["ctc"]["id"]);
